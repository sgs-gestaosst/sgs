// SGS Acesso Check — incluir em todas as páginas protegidas
...
(async function verificarAcesso(){
  ...
  try {
    ...
    const res = await fetch(
      ...
      `&select=status&order=criado_em.desc&limit=1`,
      { headers }
    );
    ...
    // Sem assinatura alguma → bug no cadastro, redireciona login
    if (!assinatura) { redirecionarLogin(); return; }

    // Free → acesso permitido (limitações aplicadas via plano_utils.js nas páginas)
    if (assinatura.status === 'Free') return;

    // Ativo ou Enterprise → acesso total
    if (assinatura.status === 'Ativo' || assinatura.status === 'Enterprise') return;

    // Suspenso ou Cancelado → bloqueia com overlay
    if (assinatura.status === 'Suspenso' || assinatura.status === 'Cancelado') {
      mostrarBloqueio('inativo');
      return;
    }

    // Trial legado ainda válido → deixa passar
    if (assinatura.status === 'Trial') {
      const fim  = new Date((assinatura.data_fim_trial || '') + 'T00:00:00');
      const hoje = new Date(); hoje.setHours(0, 0, 0, 0);
      const dias = isNaN(fim) ? 0 : Math.ceil((fim - hoje) / (1000 * 60 * 60 * 24));
      if (dias > 0) {
        if (dias <= 7) mostrarAvisoTrial(dias);
        return;
      }
      mostrarBloqueio('trial');
      return;
    }
...
function redirecionarLogin() {
  ...
  sessionStorage.removeItem('sgs_plano_cache');
  window.location.href = 'login.html';
}

function mostrarBloqueio(motivo) {
  const isTrial   = motivo === 'trial';
  const titulo    = isTrial ? 'Período de teste encerrado' : 'Assinatura inativa';
  const descricao = isTrial
    ? 'Seu período de <strong style="color:#e8eaf0;">30 dias grátis</strong> encerrou. Assine o SGS para continuar.'
    : 'Sua assinatura está suspensa ou cancelada. Regularize para continuar usando o sistema.';
  ...
  overlay.innerHTML = `
    <div style="max-width:480px;width:90%;background:#181c27;border:1px solid #2a3044;border-radius:16px;padding:40px;text-align:center;">
      ...
      <div style="font-size:20px;font-weight:600;color:#e8eaf0;margin-bottom:10px;">${titulo}</div>
      <div style="font-size:14px;color:#8892a4;line-height:1.6;margin-bottom:28px;">${descricao}</div>
...
function mostrarAvisoTrial(dias) {
  ...
  aviso.innerHTML = `
    ...
    <div>
      ...
      <div style="font-size:12px;color:#e0a855;margin-bottom:8px;">Assine para não perder o acesso.</div>
      <a href="assinar.html" style="font-size:12px;background:#e0a855;color:#0f1117;padding:5px 14px;border-radius:4px;text-decoration:none;font-weight:600;">Ver planos</a>
    </div>
